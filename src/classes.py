...
class DataHandler:
    def __init__(self, base_path):
        ...
        self.waterbackground_dir = self._find_dir("waterbackground_subtraction")
        self.high_low_stream_dir = self._find_dir("high_low_stream")
        
        
    def _find_dir(self, dir_name):
        for path in self.base_path.rglob('*'):
            if path.name == dir_name and path.is_dir():
                return path
        raise FileNotFoundError(f"{dir_name} directory not found.")   

    def _load_h5_image(self, image_name):
        image_path = self.waterbackground_dir / "images" / image_name
        with h5.File(image_path, 'r') as file:
            data = file['entry/data/data'][:]
        return data
    
    def _load_stream_data(self, stream_name):
    ...
    def _process_stream_data(self, data_columns, stream_path):
    ...
    def _compare_debug(self, high, low, *columns):
    ...
    def _retrieve_debug(self, data_columns, *columns):
